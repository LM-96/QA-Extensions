...
open class QakContext(name: String, val hostAddr: String, val portNum: Int, var mqttAddr : String = "",
    ...
    companion object {
        ...
        fun createSystem(transientSystem : TransientSystem) {
            sysUtil.createSystem(transientSystem)
            if( sysUtil.ctxOnHost.size == 0 ){
                val ip = InetAddress.getLocalHost().getHostAddress()
                sysUtil.traceprintln("               %%% QakContext | CREATING NO ACTORS on ${transientSystem.hostname} ip=${ip.toString()}")
            }
            else{
                for(scope in transientSystem.getContextsOnHost().map { it.scope }.distinct()) {
                    sysUtil.aboutThreads("QakContext scope=$scope BEFORE createContexts on ${transientSystem.hostname} ");
        ...
        @Deprecated("This method uses the old creation mechanism. Please consider using builders.",
            ReplaceWith("createSystem()"), DeprecationLevel.WARNING)
...
//			}
        ...
        @Deprecated("This method uses the old creation mechanism. Please consider using builders.",
            ReplaceWith("createSystem()"), DeprecationLevel.WARNING)
        fun createContexts(scope: CoroutineScope) {
            val hostName = sysUtil.getHostName()
            sysUtil.createContextsByAnnotation(hostName)
            if( sysUtil.ctxOnHost.size == 0 ){
                val ip = InetAddress.getLocalHost().getHostAddress()
                sysUtil.traceprintln("               %%% QakContext | CREATING NO ACTORS on $hostName ip=${ip.toString()}")
            }
...
//			runBlocking {
            sysUtil.ctxOnHost.forEach { ctx -> sysUtil.createTheActorsByAnnotation(ctx, scope)  }
            //Avoid premature termination
//            scope.launch{
//                println("               %%% QakContext |  $hostName CREATED. I will terminate after $workTime msec")
//                delay( workTime )
