...
abstract class ActorBasicFsm(  qafsmname:  String,
                      ...
                      channelSize : Int = 50, autoBuild : Boolean = true, autoStart : Boolean = true
    ...
    init {
        ...
        myself  = this
        if(autoBuild) setBody(getBody(), getInitialState(), autoStart)
	 }

    abstract fun getBody(): (ActorBasicFsm.() -> Unit)
    abstract fun getInitialState(): String

    internal fun getCurrentMessage() : IApplMessage {
        return currentMsg
    }
...
@kotlinx.coroutines.ExperimentalCoroutinesApi
    fun setBody( buildbody: ActorBasicFsm.() -> Unit, initialStateName: String, autoStart: Boolean = true ) {
...
@kotlinx.coroutines.ExperimentalCoroutinesApi
    suspend fun checkEmptyMove() {
     ...
     fun handleCurrentMessage(msg: IApplMessage, nextState: State?, memo: Boolean = true): Boolean {
        sysUtil.traceprintln("$tt ActorBasicFsmmmm $name | handleCurrentMessage in ${currentState.stateName} msg=${msg.msgId()} memo=$memo nextState=$nextState")
        if (nextState is State) {
            currentMsg   = msg
