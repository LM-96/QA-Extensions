...
@kotlinx.coroutines.ExperimentalCoroutinesApi
	...
	//val classInfos = ClassGraph().enableClassInfo().enableAnnotationInfo().scan()
...
@JvmStatic    	fun getContext( ctxName : String ) : QakContext?  { return ctxsMap[ctxName.lowercase(Locale.getDefault())]
}
@JvmStatic    	fun getActor( actorName : String ) : ActorBasic? { return ctxActorMap[actorName.lowercase(Locale.getDefault())]
}
...
@JvmStatic    	fun getActorContext ( actorName : String): QakContext?{
	...
	fun createContexts(  hostName : String,
		...
		try {
			...
			mqttBrokerPort       = solve("mqttBroker(_,PORT,_)", "PORT")?.toInt()
...
//		println("               %%% sysUtil | createFile file $file in $dir")
	...
	fun createSystem(tSystem: TransientSystem, start : Boolean = true) {
		mqttBrokerIP = null; mqttBrokerPort = null; mqttBrokerEventTopic = null
		val params = tSystem.params
		params.tryWithParam<Boolean>(KnownParamNames.TRACE) { trace = it }
			.tryWithParam<Boolean>(KnownParamNames.MSG_LOGGING) { logMsgs = it }
			.tryWithParam<String>(KnownParamNames.MQTT_IP) { mqttBrokerIP = it }
			.tryWithParam<Int>(KnownParamNames.MQTT_PORT) { mqttBrokerPort = it }
			.tryWithParam<String>(KnownParamNames.MQTT_TOPIC) { mqttBrokerEventTopic = it }
		val ctxsList = tSystem.contexts
		ctxsList.forEach { ctx -> createContext(ctx, tSystem.hostname) }
		addProxyToOtherCtxs(ctxsList.map { it.contextName }, tSystem.hostname)

		startActorsOnHost()
	}

	private fun createContext(tContext : TransientContext, hostName: String) : QakContext? {
		var mqttAddr = ""
		if( mqttBrokerIP != null ){
			mqttAddr = "tcp://$mqttBrokerIP:$mqttBrokerPort"
			println("               %%% sysUtil | context ${tContext.contextName} WORKS ALSO WITH MQTT mqttAddr=$mqttAddr")
		}
		//CREATE AND MEMO THE CONTEXT
		val newctx = if( tContext.contextAddress != hostName ){
			println("               %%% sysUtil | createTheContext ${tContext.contextName} for DIFFERENT host=${tContext.contextAddress}} ")
			QakContext( tContext.contextName, "${tContext.contextAddress}",
				tContext.contextPort, "", true) //isa ActorBasic
		}else{
			println("               %%% sysUtil | createTheContext ${tContext.contextName} FOR host=$hostName  ")
			QakContext( tContext.contextName, "${tContext.contextAddress}",
				tContext.contextPort, "") //isa ActorBasic
		}
		//val newctx = QakContext( ctx, "$ctxHost", portNum, "") //isa ActorBasic
		newctx.mqttAddr = mqttAddr //!!!!!! INJECTION !!!!!!
		ctxsMap.put(tContext.contextName, newctx)
		if( ! tContext.contextAddress.equals(hostName) ){
			return null
		}
		ctxOnHost.add(newctx)
		addTheActors(tContext, newctx)
		return newctx
	}

	private fun addTheActors(ctx: TransientContext, qakCtx : QakContext ){
		//println("sysUtil | createTheActors ${ctx.name} actorList=$actorList "   )
		ctx.actors.forEach{
			val wAct = it.wrap()
			qakCtx.addActor(wAct)
			wAct.context = qakCtx
			ctxActorMap[wAct.name] = wAct
		}
	}
	...
	/* ANNOTATION SUPPORT */
	fun createContextsByAnnotation(hostName: String){
		AnnotationLoader.scanForTraceOption().ifLoaded {
			trace = it
		}
		AnnotationLoader.scanForMsgLoggingOption().ifLoaded {
			logMsgs = it
		}
		AnnotationLoader.scanForMqqtBroker().ifLoaded {
			mqttBrokerIP = it.first
			mqttBrokerPort = it.second
			mqttBrokerEventTopic = it.third
		}.ifNotLoaded {
			mqttBrokerIP = null
			mqttBrokerPort = null
			mqttBrokerEventTopic = null
		}
		//Create the messages
		/*try {
			val dispatcNames      = solve("getDispatchIds(D)", "D")
		...
		val ctxsList = AnnotationLoader.scanForContexts().aGet()
		//waits for all the other context before activating the actors
		ctxsList.forEach { ctx -> createTheContextByAnnotation(ctx, hostName = hostName) }//foreach ctx
		//APR2020: removed, since we use CoAP e no more TCP
		addProxyToOtherCtxs(ctxsList.map { it.contextName }, hostName)  //here could wait in polling ...
	}//createContexts


	fun getHostName() : String {
		return AnnotationLoader.scanForHostname().getIfLoadedOrElse { "localhost" }
	}

	@kotlinx.coroutines.ObsoleteCoroutinesApi
	@kotlinx.coroutines.ExperimentalCoroutinesApi
	fun createTheContextByAnnotation(  ctx : it.unibo.kactor.annotations.QakContext, hostName: String  ) : QakContext?{
		val ctxName = ctx.contextName
		val ctxHost  = ctx.contextAddress
		val ctxPort = ctx.contextPort
...
//		}
		...
		if( ! ctxHost.equals(hostName) ){
			println("               %%% sysUtil | createTheContext $ctx for DIFFERENT host=$ctxHost} ")
			newctx = QakContext( ctxName, "$ctxHost", ctxPort, "", true) //isa ActorBasic
		}else{
			println("               %%% sysUtil | createTheContext $ctx FOR host=$hostName  ")
			newctx = QakContext( ctxName, "$ctxHost", ctxPort, "") //isa ActorBasic
		}
		//val newctx = QakContext( ctx, "$ctxHost", portNum, "") //isa ActorBasic
		newctx.mqttAddr = mqttAddr //!!!!!! INJECTION !!!!!!
		ctxsMap.put(ctxName, newctx)
		if( ! ctxHost.equals(hostName) ){
			return null
		}
		ctxOnHost.add(newctx)
	...
	fun createTheActorsByAnnotation( ctx: QakContext, scope : CoroutineScope ){
		val actorList = AnnotationLoader.scanForActorClassesWithSpecificContext(ctx.name)
		//println("sysUtil | createTheActors ${ctx.name} actorList=$actorList "   )
		actorList.forEach{
			createActorByAnnotation( ctx, it, scope)
		}
	}//createTheActors

	fun createActorByAnnotation( ctx: QakContext, clazz : Class<*>,
								 scope : CoroutineScope = GlobalScope  ) : ActorBasic?{
		val actor = AnnotationLoader.loadActorByClass(clazz).wrap()
		ctx.addActor(actor)
		actor.context = ctx
		//MEMO THE ACTOR
		ctxActorMap.put(actor.name,actor  )

		sysUtil.traceprintln("               %%% sysUtil | CREATE actor=${actor.name} in context:${ctx.name}  class=${clazz.name}")
			return actor
	}

	fun startAllActors() {
		for(a in ctxActorMap.values)
			if(a is ActorBasicFsm)
				a.start()
	}
